...
router.post('/create2', async function(req, res) {
  ...
  allocated_date : req.body.allocated_date, 
  shift_type :  req.body.shift_type,
  tranfer_date : req.body.tranfer_date,
  tranfer_status : req.body.tranfer_status,
        }, 
        function (err, user) {
          console.log(user)
        res.json({Status:"Success",Message:"Added successfully", Data : user ,Code:200}); 
...
router.post('/allocationdetail_remove',async function (req, res) {
        var allocation_detail = await allocation_detailModel.findOne({
  site_id:  req.body.site_id,
  emp_id : req.body.emp_id,
  tranfer_date : req.body.tranfer_date,
  tranfer_status : req.body.tranfer_status,
        ...
        if(allocation_detail !== null){
          allocation_detailModel.findByIdAndRemove(allocation_detail._id, function (err, user) {
...
router.get('/update_params',async function (req, res) {
        allocation_detailModel.find({},async function (err, Functiondetails) {
          for(let a = 0; a < Functiondetails.length; a++){
           allocation_detailModel.findByIdAndUpdate(Functiondetails[a]._id,{tranfer_date : '',tranfer_status : ''}, {new: true}, function (err, UpdatedDetails) {
            if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500}); 
            });
           if(a == Functiondetails.length - 1 ){
            res.json({Status:"Success",Message:"allocation detail  Updated", Data : {} ,Code:200});
           }
          }
        }).sort({_id:-1});
});
